# backend/app/collectors/austria/willhaben.py
...
class WillhabenCollector(JobCollector):
    """Collector for Willhaben job listings (similar to Kleinanzeigen)"""
    
    BASE_URL = "https://www.willhaben.at"
    SEARCH_URL = "https://www.willhaben.at/iad/jobs/stellenangebote"  # Korrigierte URL
    
    def __init__(self):
        super().__init__()
        self.source = "Willhaben"
        self.headers = {
    ...
    def fetch_jobs(self, filter_params: Optional[Any] = None) -> List[Dict[str, Any]]:
        """
        Fetch jobs from Willhaben
        """
        print(f"[WillhabenCollector] START fetch_jobs")
        ...
        print(f"[WillhabenCollector] END - found {len(jobs)} jobs")
        return jobs[:limit]
    
    def _fetch_page(self, keyword: str, location: str, page: int) -> List[Dict[str, Any]]:
        """Fetch a single page of results from Willhaben"""
        params = {
            "page": page,
        }
        ...
        try:
            print(f"[WillhabenCollector] Fetching URL: {self.SEARCH_URL}")
            print(f"[WillhabenCollector] Params: {params}")
            ...
            print(f"[WillhabenCollector] Response Status: {response.status_code}")
            
            if response.status_code == 200:
                with open(f"willhaben_page_{page}.html", "w", encoding="utf-8") as f:
            ...
            print(f"[WillhabenCollector] Error: {e}")
            return []
    
    def _parse_jobs_page(self, html: str) -> List[Dict[str, Any]]:
        """Parse job listings from Willhaben HTML"""
        soup = BeautifulSoup(html, 'html.parser')
        jobs = []
        
        # Find job listings
        job_cards = soup.select('article[data-testid="job-card"], div.job-tile, div[data-testid="result-item"]')
        
        if not job_cards:
            job_cards = soup.select('div.job-item, div.listing-item, div.result-card')
        
        print(f"[WillhabenCollector] Found {len(job_cards)} job cards")
        ...
        for card in job_cards:
            try:
                ...
                print(f"[WillhabenCollector] Error parsing job card: {e}")
                continue
    ...
    def _extract_job_from_card(self, card: BeautifulSoup) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Title
            title_elem = card.select_one('h2 a, h3 a, a[data-testid="job-title"], a.job-title')
            if title_elem:
                ...
                title_elem = card.select_one('[data-testid="title"], .title')
                if title_elem:
                    title = title_elem.get_text(strip=True)
                    url = None
                else:
            ...
            # Date
            date_elem = card.select_one('[data-testid="date"], .date, time')
            if date_elem:
            ...
            print(f"[WillhabenCollector] Error extracting job: {e}")
            return None
    ...
    def _get_fallback_jobs(self, keyword: str, location: str, limit: int) -> List[Dict[str, Any]]:
        ...
        for i in range(min(max(limit, 1), 5)):
            jobs.append({
                "title": f"Willhaben Fallback {i+1}: {keyword or 'Stelle'}",
                "company": "Austrian Employer",
                "city": location or "Wien",
                "date": datetime.now().isoformat(),
                "salary_min": 2000 + i * 300,
